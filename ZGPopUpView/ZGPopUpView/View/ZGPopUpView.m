...
//        NSLog(@"%@---",NSStringFromCGRect(self.frame));
        ...
        self.frame = CGRectMake(contentViewLeftInset + popUpViewInset, popUpViewInset + contentViewTopInset, size.width, size.height);//文字视图的坐标尺寸
//        NSLog(@"%@---",NSStringFromCGRect(self.frame));
    }
}
...
- (void)showMessage:(NSString *)message attributes:(NSDictionary *)attributes inView:(UIView *)view rect:(CGRect)rect{
    if (message.length > 0) {
        ...
        if (rect.origin.y +rect.size.height + textRect.size.height >([UIScreen mainScreen].bounds.size.height -5)) {
            self.arrowDirection= ZGPopUpViewArrowDirectionDown;
        }
        if (rect.origin.y - textRect.size.height <5){
            self.arrowDirection= ZGPopUpViewArrowDirectionUp;
        }
...
+ (void)showView:(UIView *)aShowView inView:(UIView *)view rect:(CGRect)rect{
    if (_popUpView_.window) return;
    ZGPopUpView *popUpView = [[self alloc] init];
    popUpView.backgroundColor = [UIColor cyanColor];
    [popUpView showView:aShowView inView:view rect:rect];
}
- (void)showView:(UIView *)aShowView inView:(UIView *)view rect:(CGRect)rect{
    if (_popUpView_.window) return;
    CGRect textRect = aShowView.bounds;
    ...
    [self.contentView addSubview:aShowView];
...
- (CGRect)popUpViewFrameWithView:(UIView *)view rect:(CGRect)rect{
    ...
    NSLog(@"%.2f--arrowPointX-",arrowPointX);
    ...
    CGPoint arrowPoint = CGPointMake(arrowPointX, arrowPointY);//获取箭头点
    if (arrowPointX + popUpViewWidth/2<=[UIScreen mainScreen].bounds.size.width &&(arrowPointX-popUpViewWidth/2) >=0) {
        self.arrowPoint = arrowPoint;
        UIView *redView = [[UIView alloc] initWithFrame:CGRectMake(arrowPoint.x, arrowPoint.y-5, 5, 5)];
        redView.backgroundColor = [UIColor redColor];
        self.maskView.frame = view.bounds;
        [view addSubview:self.maskView];
        [view addSubview:self];
        [view addSubview:redView];
        rightExtendDistance = popUpViewWidth/2-arrowRadius-cornerRadius;
        if (self.arrowDirection == ZGPopUpViewArrowDirectionUp) {
             return CGRectMake(arrowPointX-popUpViewWidth/2, arrowPointY, popUpViewWidth, popUpViewHeight);
        }else{
            return CGRectMake(arrowPointX-popUpViewWidth/2, arrowPointY-popUpViewHeight, popUpViewWidth, popUpViewHeight);
        }
    }else{
        // rightExtendDistance 右边扩展距离
        CGFloat rightSpace = view.bounds.size.width - arrowPoint.x;
        if (rightSpace == view.bounds.size.width / 2.0) {//中间
            rightExtendDistance = (popUpViewWidth - 2 * (arrowRadius + cornerRadius)) / 2.0;
        }else if (rightSpace > view.bounds.size.width / 2.0){//箭头靠左
        ...
        CGFloat pointX = arrowPoint.x - (popUpViewWidth - arrowRadius - rightExtendDistance - cornerRadius  - popUpViewInset);
        if (pointX < 5 && rect.origin.x < [UIScreen mainScreen].bounds.size.width) {
            //        rightExtendDistance = rectWidth;
            CGFloat pan = 5 - pointX;
            rightExtendDistance += pan;
            pointX = 5;
        }else if (pointX + popUpViewWidth > [UIScreen mainScreen].bounds.size.width -5){
            rightExtendDistance -=5;
            pointX -=5;
        }
        CGRect popUpViewFrame = CGRectMake(pointX, popUpViewY, popUpViewWidth, popUpViewHeight);
        self.arrowPoint = arrowPoint;
        self.maskView.frame = view.bounds;
        [view addSubview:self.maskView];
        [view addSubview:self];
